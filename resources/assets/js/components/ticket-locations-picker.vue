...
export default {
    ...
    methods: {
        ...
        select(locationData, location) {
            location.id = null;
            location.longname = locationData.formatted;
            location.lat = +locationData.geometry.lat;
            location.lng = +locationData.geometry.lng;
        ...
        updateInput: debounce(function(location) {
            ...
            $.getJSON(config.geocoderUrl + location.name, response => {
                console.log('geocoding request sent, results:');
                console.log(response.results);
                if (response.results.length === 0) return;

                location.geocoderResults = response.results.slice(0, 5);
            });
        }, 1000),
